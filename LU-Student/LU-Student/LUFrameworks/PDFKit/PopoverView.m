...
#define _POPOVER_ARROW_HEIGHT 20.0
#define _POPOVER_ARROW_BASE 20.0
#define _POPOVER_RADIUS 10.0
...
-(void)setBorder:(BOOL)border
{
    ...
    if(self.border  ==  NO) {
        ...
        _contentView.layer.cornerRadius  =  _POPOVER_RADIUS;
    }
}
...
-(CGPathRef)newContentPathWithBorderWidth:(CGFloat)borderWidth arrowDirection:(PopoverArrowDirection)direction
{
    CGFloat w  =  self.bounds.size.width;
    CGFloat h  =  self.bounds.size.height;
    CGFloat ah  =  _POPOVER_ARROW_HEIGHT; //is the height of the triangle of the arrow
    CGFloat aw  =  _POPOVER_ARROW_BASE/2.0; //is the 1/2 of the base of the arrow
    CGFloat radius  =  _POPOVER_RADIUS;
...
-(CGGradientRef)newGradient
{
    ...
    else if(self.tint  ==  PopoverWhiteTint)
    {
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGPoint start;
    CGPoint end;
    if(_arrowDirection  ==  PopoverArrowDirectionUp || _arrowDirection  ==  PopoverNoArrow)
    {
...
-(void)setupViews
{
    ...
    {
        contentRect.origin  =  CGPointMake(10 + _POPOVER_ARROW_HEIGHT, 40);        
        contentRect.size  =  CGSizeMake(self.bounds.size.width-40, self.bounds.size.height-50);
        _titleLabel.frame  =  CGRectMake(10, 10, self.bounds.size.width-20, 20); 
		if (self.title == nil || self.title.length == 0) {
			contentRect.origin  =  CGPointMake(10+ _POPOVER_ARROW_HEIGHT, 10);
			contentRect.size  =  CGSizeMake(self.bounds.size.width-40, self.bounds.size.height-20);
		}
    }
    
    else if(_arrowDirection  ==  PopoverNoArrow)
    {
        contentRect.origin  =  CGPointMake(10, 40);
