...
#pragma mark - ContentPage class methods
...
#pragma mark - ContentPage PDF link methods
...
- (void)highlightPageLinks
{
	if (_links.count > 0) // Add highlight views over all links
	{
		...
		for (DocumentLink *link in _links) // Enumerate the links array
		{
...
- (DocumentLink *)linkFromAnnotation:(CGPDFDictionaryRef)annotationDictionary
{
	DocumentLink *documentLink  =  nil; // Document link object
	...
	if (CGPDFDictionaryGetArray(annotationDictionary, "Rect", &annotationRectArray))
	{
		...
		documentLink  =  [DocumentLink newWithRect:viewRect dictionary:annotationDictionary];
	}
		...
		for (NSInteger index  =  0; index < count; index++) // Iterate through all annotations
		{
			...
			if (CGPDFArrayGetDictionary(pageAnnotations, index, &annotationDictionary)  ==  true)
			{
				...
				if (CGPDFDictionaryGetName(annotationDictionary, "Subtype", &annotationSubtype)  ==  true)
				{
					...
					{
						DocumentLink *documentLink  =  [self linkFromAnnotation:annotationDictionary];
...
- (id)processSingleTap:(UITapGestureRecognizer *)recognizer
{
	...
	if (recognizer.state  ==  UIGestureRecognizerStateRecognized)
	{
		if (_links.count > 0) // Process the single tap
		{
			...
			for (DocumentLink *link in _links) // Enumerate links
			{
...
#pragma mark - ContentPage instance methods
...
- (instancetype)initWithURL:(NSURL *)fileURL page:(NSInteger)page password:(NSString *)phrase
{
	...
	ContentPage *view  =  [self initWithFrame:viewRect];
...
- (void)saveImagetoPDF: (DrawingView *)drawingView withURL:(NSURL *)destURL
{
...
#if (_DISABLE_RETINA  ==  TRUE) // Option
...
#endif // end of _DISABLE_RETINA Option

#pragma mark - CATiledLayer delegate methods

- (void)drawLayer:(CATiledLayer *)layer inContext:(CGContextRef)context
{
	ContentPage *ContentPage  =  self; // Retain self
...
//
//	DocumentLink class implementation
//
...
#pragma mark - DocumentLink class methods

+ (instancetype)newWithRect:(CGRect)linkRect dictionary:(CGPDFDictionaryRef)linkDictionary
{
	return [[DocumentLink alloc] initWithRect:linkRect dictionary:linkDictionary];
}

#pragma mark - DocumentLink instance methods
