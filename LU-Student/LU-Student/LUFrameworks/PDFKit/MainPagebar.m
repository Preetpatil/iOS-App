...
#pragma mark - MainPagebar class methods
...
#pragma mark - MainPagebar instance methods
...
- (void)updatePageThumbView:(NSInteger)page
{
	...
	if (page !=  pageThumbView.tag) // Only if page number changed
	{
		...
		ThumbRequest *request  =  [ThumbRequest newForView:pageThumbView fileURL:fileURL password:phrase guid:guid page:page size:size];

		UIImage *image  =  [[ThumbCache sharedInstance] thumbRequest:request priority:YES]; // Request the thumb
...
{
	assert(object !=  nil); // Must have a valid Document
	...
	if ((self  =  [super initWithFrame:frame]))
	{
		...
		trackControl  =  [[TrackControl alloc] initWithFrame:self.bounds]; // Track control view
...
- (void)layoutSubviews
{
	...
	if (pageThumbView  ==  nil) // Create the page thumb view when needed
	{
		...
		pageThumbView  =  [[PagebarThumb alloc] initWithFrame:thumbRect]; // Create the thumb view
	...
	for (NSInteger thumb  =  0; thumb < thumbs; thumb++) // Iterate through needed thumbs
	{
		...
		PagebarThumb *smallThumbView  =  [miniThumbViews objectForKey:key]; // Thumb view
		...
		if (smallThumbView  ==  nil) // We need to create a new small thumb view for the page number
		{
			...
			smallThumbView  =  [[PagebarThumb alloc] initWithFrame:thumbRect small:YES]; // Create a small thumb view

			ThumbRequest *thumbRequest  =  [ThumbRequest newForView:smallThumbView fileURL:fileURL password:phrase guid:guid page:page size:size];

			UIImage *image  =  [[ThumbCache sharedInstance] thumbRequest:thumbRequest priority:NO]; // Request the thumb
	...
	[thumbsToHide enumerateKeysAndObjectsUsingBlock: // Hide unused thumbs
		...
		{
			PagebarThumb *thumb  =  object; thumb.hidden  =  YES;
		}
	];
}
...
#pragma mark - TrackControl action methods
...
//
//	TrackControl class implementation
//
...
#pragma mark - TrackControl instance methods
...
//
//	PagebarThumb class implementation
//

@implementation PagebarThumb

#pragma mark - PagebarThumb instance methods
...
//
//	PagebarShadow class implementation
//

@implementation PagebarShadow

#pragma mark - PagebarShadow class methods
...
#pragma mark - PagebarShadow instance methods
