...
@interface ThumbsView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
#pragma mark - ThumbsView instance methods
...
- (void)requeueThumbCell:(ThumbView *)tvCell
{
...
- (void)requeueAllThumbCells
{
	if (thumbCellsVisible.count > 0)
	{
		...
		for (ThumbView *tvCell in visible)
		{
...
- (ThumbView *)dequeueThumbCellWithFrame:(CGRect)frame
{
	ThumbView *theCell  =  nil;
	...
	else // Allocate a brand new thumb cell subclass for our use
	{
		...
		//assert([theCell isKindOfClass:[ThumbView class]]);
...
- (ThumbView *)thumbCellContainingPoint:(CGPoint)point
{
	ThumbView *theCell  =  nil;

	for (ThumbView *tvCell in thumbCellsVisible)
	{
...
- (void)layoutSubviews
{
	...
	if (CGSizeEqualToSize(_lastViewSize, self.bounds.size)  ==  false)
	{
		...
		for (ThumbView *tvCell in thumbCellsVisible) // Enumerate visible cells
		{
		...
		for (ThumbView *tvCell in requeueCells) // Enumerate requeue cells
		{
		...
		[visibleIndexSet enumerateIndexesUsingBlock: // Enumerate visible indexes
			^(NSUInteger index, BOOL *stop)
			{
				...
				ThumbView *tvCell  =  [self dequeueThumbCellWithFrame:thumbRect];
...
- (void)handleTapGesture:(UITapGestureRecognizer *)recognizer
{
	if (recognizer.state  ==  UIGestureRecognizerStateRecognized) // Handle the tap
	{
		...
		ThumbView *tvCell  =  [self thumbCellContainingPoint:point]; // Look for cell
